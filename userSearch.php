...
if ($data) {
    ...
    // $userId = 27;
    $sql = "SELECT id,username,image FROM users WHERE (username like '%$text%' and id!=$userId)";
    ...
    if ($rowcount > 0) {
        ...
        while ($row2 = mysqli_fetch_assoc($result)) {
            ...
            array_push($users, [$row2['id'], $row2['username'], $img]);
        }
        // print_r($users);
        echo json_encode(["status" => 1, "data" => $users, "message" => "users exist"]);
    } else {
